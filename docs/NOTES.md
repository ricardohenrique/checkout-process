...
- **Queue driver and webhook latency.** The implementation uses `QUEUE_CONNECTION=sync` (the project default), which runs listeners inline during the request. The spec requires the webhook to respond in under 500ms; this guarantee only holds with an async driver (e.g. `database` or Redis) and a dedicated `artisan queue:work` process. Under `sync`, the warehouse HTTP call (2–3s, 10s timeout) runs inside the webhook request and will exceed that budget. Switching to an async driver is the correct production path — the architecture already supports it, as all listeners implement `ShouldQueue`. The sync driver is used here because it makes the test suite deterministic and requires no extra infrastructure.
